...
impl ClientBuilder {
    ...
    pub async fn build(self) -> Result<ResponsesClient<rig_reqwest::ReqwestClient>, MantleError> {
        let (api_key, base_url) = self.resolve_auth().await?;
        Ok(ResponsesClient::builder()
            .api_key(api_key)
            .base_url(base_url)
            .http_client(rig_reqwest::ReqwestClient(reqwest::Client::new()))
            .build()?)
    }
    ...
    /// The bearer token is snapshotted into the client. See [`TOKEN_TTL`] and
    /// [`effective_token_ttl`]. Defaults never point at `api.openai.com`.
    pub async fn build_completions(self) -> Result<CompletionsClient<rig_reqwest::ReqwestClient>, MantleError> {
        let (api_key, base_url) = self.resolve_auth().await?;
...
pub async fn from_env() -> Result<ResponsesClient<rig_reqwest::ReqwestClient>, MantleError> {
...
pub async fn from_env_completions() -> Result<CompletionsClient<rig_reqwest::ReqwestClient>, MantleError> {
